...
public class ObjectRenderer {
   ...
   * オブジェクトモデル行列を更新し、スケーリングを適用します。
   *    *
   *    * 4x4 のモデルからワールドへの変換行列．
   *    * @param scaleFactor {@code modelMatrix}の前に適用される個別のスケーリング係数．
   *    * @param scaleFactor {@code modelMatrix}の前に適用される個別のスケーリング係数．
   */
  ...
  public void rotate(float[] modelMatrix){
    while(true) {
      Matrix.rotateM(this.modelMatrix, 0, 180, 1f, 0f, 0f);
    }
  }
}
